...
def test_reconcile_withdraw_strategies():
    ...
    plugin._withdraw_from_source(fifo, Decimal("300"))
    ...
    plugin._withdraw_from_source(gain_first, Decimal("300"))
    ...
    plugin._withdraw_from_source(pro_rata, Decimal("200"))
...
def test_reconcile_records_realized_gain():
    raw = _minimal_config_dict()
    config = AppConfig(**raw)
    plugin = ReconcilePlugin(config)
    ...
    state = SimulationState(
        ...
        capital_sources={"depot": source},
        inflows={},
        outflows={"costs": LedgerEntry(value=Decimal("300"), label="Kosten")},
        deductions={},
        capital_withdrawal_order=["depot"],
    )
    state.compute_summaries()

    state = plugin.execute(state, None)

    assert "realized_gain_depot" in state.inflows
    assert state.inflows["realized_gain_depot"].value == Decimal("300")
    assert state.capital_sources["depot"].capital_total == Decimal("700")
